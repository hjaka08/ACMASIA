...
\acmConference[ASIA CCS '26]{ACM Asia Conference on Computer and Communications Security}{June 01--05, 2026}{Bangalore, India}
%%
...
We consider an attacker who is on the same optical splitter as the victim.
\begin{itemize}
  \item \textbf{1. Premise} The attacker has an optical capture device (e.g., an FPGA with GPON optics or a modified ONU capable of logging raw GTC frames) connected to the splitter and passively records the downstream broadcast from the OLT.
The attacker does not need to register their device with the OLT; thus, a receive-only optical tap can record the downstream stream without transmitting.
We assume the attacker can perform the necessary physical-layer processing to recover GTC frames (clock recovery, frame delineation, de-scrambling, and FEC decode) with sufficient lock time and bit error rate for header parsing.
  ...
  \emph{Optional:} if the attacker additionally knows (or can learn via a side channel) the Port-ID used by a specific victim's ONU/service, they can filter the dataset to that \texttt{port\_id} and perform victim-specific inference.
...
First, we played each of 10 videos (three runs each; run1, run2, run3) on Ubuntu 24.04 under a fixed playback quality and resolution setting (720p). To minimize interference, we restricted other background traffic (e.g., VNC, SSH) during playback. For each run, we captured downlink ethernet packet (PCAP) for 180 seconds playback with a $\pm 3\,s$ buffer. Then, we replayed each PCAP file in our simulator to produce a CSV dataset of PLI values and timestamps(one trace per run) (e.g., vid001\_run001.gem.csv~\cite{github}). We preserve original PCAP packet timestamps to retain inter-burst timing gaps; output traces are sparse (idle bins omitted) but can be reconstructed by binning using t\_start\_us; each synthesized GEM is assigned t\_start\_us derived from the PCAP time base (µs resolution). After producing the dataset, we visualized cumulative PLI--Time Plot based on our dataset(Publicly Available on our Github repository)~\cite{github}. We observed a clear correlation between each video and its cumulative PLI over time graph(Fig 3.)~\cite{github}; traces from the same video show highly consistent slopes across runs.(uploaded our Github repository's directory: \//out\_cumsum)
...
\begin{figure}[!t]
  \centering
  \includegraphics[width=0.90\linewidth]{fig/box_rmse.png}
  \vspace{-0.3em}
  \includegraphics[width=0.90\linewidth]{fig/box_pearson_r.png}
  \caption{Two stacked box plots: RMSE (lower better), Pearson r (higher better)}
...
\begin{figure}[!t]
  \centering
  \includegraphics[width=0.85\linewidth]{fig/cumsum_vid001.pdf}
...
\end{itemize}
\noindent\textbf{Why this still matters:} even without a full testbed, the simulator isolates the information leakage inherent to plaintext GEM metadata (PLI) and quantifies its discriminative power under controlled, repeatable conditions.

\textbf{Future Work.}
We will validate PLI-based leakage and identification on a physical GPON testbed with a commercial OLT/ONU and an optical splitter.
Using a FPGA based capture device, we will passively extract downstream GTC/GEM headers and construct PLI time series from real deployments.
We will also expand the dataset to larger video catalogs and longer term measurements and adaptive streaming dynamics (e.g., resolution/bitrate changes). 

\emph{Receive-only GPON header parsing requires:}
(i) clock recovery, (ii) frame delineation, (iii) de-scrambling, (iv) FEC decode. We are building an FPGA based tap and will report feasibility + BER/lock time on commercial OLT and ONU
...
\bibitem{github}
Anonymized link of Our github repository. \url{https://anonymous.4open.science/r/GPON-2195}. Accessed 2026-02-04.
