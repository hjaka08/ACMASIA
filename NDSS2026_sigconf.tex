...
While prior studies primarily focused on upper layer or cellular traffic analysis~\cite{bae-sec22}~\cite{ytyt} , we researched whether the plaintext PLI field in GEM headers can be exploited to infer a victim's streaming activity. Complementary to upper-layer video fingerprinting, we study an access-layer observable (GEM PLI) that survives payload encryption in GPON. 
...
\end{itemize}
\textbf{Note.}Reliable receive-only parsing likely requires optical/protocol synchronization (clock recovery, frame delineation, de-scrambling, and FEC decoding); we leave hardware validation to future testbed work.
\begin{figure}[h]
  \centering
  \vspace{-0.2em}
  \includesvg[
    ...
    inkscapelatex=false
  ]{fig/pip_with_screenshot_compact_readable}
...
\begin{itemize}
  \item \textbf{Modeled (per G.984.3)} 125µs framing, GEM header fields, fragmentation to satisfy 12-bit PLI
  \item \textbf{May vary by vendor} fragmentation policy / padding / FEC / scheduling jitter
  \item \textbf{Impact on leakage} affects PLI dynamics → motivates real OLT/ONU capture (future work)
\end{itemize}
...
\begin{figure}[!t]
  \centering
  \includegraphics[width=0.95\linewidth]{fig/box_rmse.png}
  \vspace{-0.3em}
  \includegraphics[width=0.95\linewidth]{fig/box_pearson_r.png}
  \caption{Similarity distributions between same-video and different-video cumulative PLI traces (RMSE and Pearson correlation). Same video pairs tend to show higher correlation and smaller RMSE distance; we found a partial intersection due to ABR variability and buffering, but the distinction is sufficient for discrimination using both metrics.}
  \Description{Two stacked box plots showing similarity metrics between same-video and different-video cumulative PLI traces: RMSE (lower is more similar) and Pearson correlation (higher is more similar).}
...
\begin{figure}[!t]
  ...
  \caption{PLI - Time graph for one of 10 videos}
  \Description{PLI - Time graph for one of 10 videos showing that highly identical slopes between three runs. PLI - Time Graph of others is publicly available on our github repository}
  \label{fig:plitime}
\end{figure}
...
\textbf{Future Work} will be conducted in testbed with Real OLT, ONU and Optical Splitter. We will use FPGA device to capture GEM frames to parse downstream frames. We will expand to more videos and longer captures with changing various resolution, bitrate. 

Planned robustness checks: (i) first 30/60/90s identification, (ii) ±Δt start-time shifts, (iii) background traffic on same Port-ID, (iv) ABR auto-quality.
